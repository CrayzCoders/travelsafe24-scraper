...
async function main() {
  ...
  console.log(
    `[clean]   price=${stats.withPrice} (${pct(stats.withPrice, stats.total)})`,
  );
  console.log(
    `[clean]   rooms=${stats.withRooms} (${pct(stats.withRooms, stats.total)})`,
  );
  console.log(
    `[clean]   size=${stats.withSize} (${pct(stats.withSize, stats.total)})`,
  );
  console.log(
    `[clean]   district=${stats.withDistrict} (${pct(stats.withDistrict, stats.total)})`,
  );
  if (stats.unknownDistricts.length > 0) {
    console.warn(
      `[clean] unknown districts (add alias): ${stats.unknownDistricts.join(", ")}`,
    );
  }
